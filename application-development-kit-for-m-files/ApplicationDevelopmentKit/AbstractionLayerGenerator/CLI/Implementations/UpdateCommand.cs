...
namespace ApplicationDevelopmentKit
{
	public class UpdateCommand : ICommand
	{
		...
		public UpdateCommand(CommandLineOptions commandLineOpts,
			string projectName,
			string serverName,
			string port,
            string vaultName,
		...
		{
			MFilesSettings mfSettings = new MFilesSettings() { MFDevToolCommand = MFDevelopmentKitCommand.Update };
			...
			if (!IsEmpty()) {
				mfSettings = new MFilesSettings() {
					...
					, Username = _UserName
					, Password = _Password
					, MFDevToolCommand = MFDevelopmentKitCommand.Update
			...
			//AbstractMFDeveloperTool developerTool = new CLIDeveloperTool();
			//developerTool.MFilesSettings = mfilesSettings;
			//Api api = new Api(developerTool);
